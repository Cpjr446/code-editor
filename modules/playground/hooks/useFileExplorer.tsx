...
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  ...
  openFile: (file, filePath) => {
    const fileId = filePath || generateFileId(file, get().templateData!);
    const { openFiles } = get();
  ...
  handleAddFile:async(newFile , parentPath , writeFileSync , instance , saveTemplateData)=>{
    ...
    try {
      ...
      const fileId = parentPath ? `${parentPath}/${file.filename}.${file.fileExtension}` : `${file.filename}.${file.fileExtension}`;
    ...
    try {
      ...
      const closeFilesInFolder = (folder: TemplateFolder, currentPath: string = "") => {
        folder.items.forEach((item) => {
          if ("filename" in item) {
            const fileId = currentPath ? `${currentPath}/${item.filename}.${item.fileExtension}` : `${item.filename}.${item.fileExtension}`;
            get().closeFile(fileId);
   ...
   handleRenameFile: async (
    ...
    // Generate old and new file IDs using parentPath and filename
    const oldFileId = parentPath ? `${parentPath}/${file.filename}.${file.fileExtension}` : `${file.filename}.${file.fileExtension}`;
    const newFileId = parentPath ? `${parentPath}/${newFilename}.${newExtension}` : `${newFilename}.${newExtension}`;
